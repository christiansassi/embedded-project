...
//*****************************************************************************
//
//! Initialization of Analog-to-Digital Converter (ADC)
//
//*****************************************************************************

void _adcInit()
{
    /* Configures Pin 6.1 (x-axes) as ADC input*/
    GPIO_setAsPeripheralModuleFunctionInputPin(GPIO_PORT_P6, GPIO_PIN1,GPIO_TERTIARY_MODULE_FUNCTION);

    /* Configures Pin 4.4 (y-axes) as ADC input */
    ...
    /* Configuring ADC Memory (ADC_MEM0 - ADC_MEM1 (A14, A9)  with no repeat) with 3.3v reference */
...
void _encoderInit()
{
    ...
    GPIO_setAsInputPinWithPullUpResistor(portDt, pinDt);     //P4.2
}
