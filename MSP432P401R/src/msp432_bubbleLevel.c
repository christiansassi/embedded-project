...
void bubble(Graphics_Context *g_sContext)
{
    ...
    angle = (angle > MAX_ANGLE + CALIBRATION_OFFSET) ? MAX_ANGLE + CALIBRATION_OFFSET : angle;

    // Keep pos between min and max
    ...
    if (count == N_CAPTURES)
    {
        ...
        char angleString[5];
        sprintf(angleString, "%0.2f deg", angle);
...
void bubbleInit(Graphics_Context *g_sContext)
{
    ...
    Graphics_drawStringCentered(g_sContext, (int8_t *) "        ", AUTO_STRING_LENGTH, 64, 90, OPAQUE_TEXT);
    Graphics_drawStringCentered(g_sContext, (int8_t *) "0.00 deg", AUTO_STRING_LENGTH, 64, 90, OPAQUE_TEXT);
