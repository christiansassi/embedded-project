...
The `encoderInit(...)` function is first called to turn off the LED and display the initial interface. Then, the function `meter(...)` starts the measurement. It's possible to return to the menu by pressing the `B` button.
...
The `lidarInit(...)` function is first called to turn off the LED and display the initial interface. Then, the function `lidar(...)` starts the measurement. It's possible to return to the menu by pressing the `B` button.
